...
void SATSolverDPLL::initialize() {
  ...
  //   }
  // }
  cout<<"Enter the inputs for only the CNF form "<<endl;
  cout<<"Enter the number of different literals:";
  cin >> literal_count;
 ...
 * of the Ans enum Ans::satisfied - the formula has been satisfied
 *               Ans::unsatisfied - the formula can no longer be satisfied
 *               Ans::normal - normal exit
 */
int SATSolverDPLL::unit_propagate(Formula &f) {
  ...
  {
    return Ans::satisfied; // it is vacuously satisfied
  }
  do {
    ...
    for (int i = 0; i < f.clauses.size(); i++) {
      if (f.clauses[i].size() ==1) // if the size of a clause is 1, it is a unit clause
      {
        ...
        if (result == Ans::satisfied || result == Ans::unsatisfied) {
          return result;
        }
        break; // exit the loop to check for another unit clause from the start
      } else if (f.clauses[i].size() == 0) // if a given clause is empty
      {
        return Ans::unsatisfied; // the formula is unsatisfiable in this branch
      }
    }
  } while (unit_clause_found);

  return Ans::normal; // if reached here, the unit resolution ended normally
}
 ...
 * return value: int - the return status flag, a member of the Ans enum
 *               Ans::satisfied - the formula has been satisfied
 *               Ans::unsatisfied - the formula can no longer be satisfied
 *               Ans::normal - normal exit
 */
 ...
 * return value: int - the return status flag, a member of the Ans enum
 *               Ans::normal - exited normally
 *               Ans::completed - result has been found, exit recursion all the
 * way
 */
int SATSolverDPLL::DPLL(Formula f) {
  int result = unit_propagate(f); // perform unit propagation on the formula
  if (result == Ans::satisfied) // if formula satisfied, show result and return
  {
    show_result(f, result);
    return Ans::completed;
  } else if (result == Ans::unsatisfied) // if formula not satisfied in this
  ...
  for (int j = 0; j < 2; j++) {
    ...
    if (transform_result ==
        Ans::satisfied) // if formula satisfied, show result and return
    {
      show_result(new_f, transform_result);
      return Ans::completed;
    } else if (transform_result == Ans::unsatisfied) // if formula not satisfied
    ...
    if (dpll_result == Ans::completed) // propagate the result, if completed
    {
      return dpll_result;
    }
  }
 ...
 *            result - the result flag, a member of the Ans enum
 */
void SATSolverDPLL::show_result(Formula &f, int result) {
  if (result == Ans::satisfied) // if the formula is satisfiable
  {
    cout << "SAT" << endl;
...
void SATSolverDPLL::solve() {
  ...
  if (result == Ans::normal) {
    show_result(formula, Ans::unsatisfied); // the argument formula is a dummy
